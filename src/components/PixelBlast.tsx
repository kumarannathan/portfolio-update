...
const createTouchTexture = (): TouchTexture => {
  ...
  const update = () => {
    ...
    for (let i = trail.length - 1; i >= 0; i--) {
      const point = trail[i];
      const f = point.force * pointSpeed * (1 - point.age / maxAge);
      point.x += point.vx * f;
      point.y += point.vy * f;
      point.age++;
...
void main() {
  ...
  vec2 pixelId = floor(fragCoord / pSize);
  vec2 pixelUV = fract(fragCoord / pSize);

  float cellPixelSize = 8.0 * pSize;
  ...
  float bayer = Bayer8(fragCoord / pSize) - 0.5;
  float bw = step(0.5, feed + bayer);

  float h = fract(sin(dot(floor(fragCoord / pSize), vec2(127.1, 311.7))) * 43758.5453);
    ...
    if (mustReinit) {
      ...
      if (noiseAmount > 0) {
        ...
        composer.passes.forEach((pass) => {
          const p = pass as { renderToScreen?: boolean };
      ...
      const animate = () => {
        ...
        if (composer) {
          ...
          composer.passes.forEach((pass) => {
            const p = pass as { effects?: Array<Effect & { uniforms: Map<string, THREE.Uniform> }> };
